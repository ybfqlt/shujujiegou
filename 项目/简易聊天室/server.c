...
//保存被禁言的人
typedef struct nosay
{
    char name[20];  //保存被禁言的人
    char qunname[20]; //保存实施禁言的群
    struct nosay *next;
}jinyan;
...
void chat_group(packet *recv)
{
    ...
    while(p)
    {
        if(strcmp(p->groupname,recv->data.name[0])==0)
        {
            ...
            while(pt)
            {
                if(strcmp(recv->data.name[0],pt->qunname)==0)
                {
                    if(strcmp(recv->data.from,pt->name)==0)
                    {
                        jin=1;
                    }
                }
...
void deal_reqsendfile(packet *recv)
{
    ...
    q=(refuse *)malloc(sizeof(refuse));
    strcpy(q->from,recv->data.to);
    strcpy(q->to,recv->data.from);
    strcpy(q->file,recv->data.message);
    if(recv->data.name[0][0]=='y')
    {
        q->sign=1;
    }
...
void jinyanone(packet *recv)
{
    ...
    while(p)
    {
        if(strcmp(p->groupname,recv->data.message)==0);
        {
            if(strcmp(p->groupowner,recv->data.from)==0)
            {
                flag=1;
                qt=(jinyan *)malloc(sizeof(jinyan));
                strcpy(qt->qunname,recv->data.message);
                strcpy(qt->name,recv->data.to);
                q=jinhead;
                nq=q;
                while(q)
                {
                ...
                for(int j=0;j<p->membernum;j++)
                {
                    ...
                    while(h)
                    {
                        if(strcmp(h->username,p->member[j])==0)
                        {
                            recv->type='$';
                            recv->data.recvfd=h->conn_fd;
                            break;
                        }
                        h=h->next;
                    }
                ...
                printf("%s被群主%s禁言%d秒\n",recv->data.to,recv->data.from,recv->data.size);
                sleep(recv->data.size);
                if(qt->next==NULL)
                {
                    free(qt);
                }
                ...
                for(int i=0;i<p->membernum;i++)
                {
                    ...
                    while(h)
                    {
                        if(strcmp(h->username,p->member[i])==0)
                        {
                            recv->type='@';
                            recv->data.recvfd=h->conn_fd;
                            break;
...
//从群成员中移除某人
void remove_one(packet *recv)
{
    int groupn=0;
    int biaof=0;
    int biaot=0,j,te,locate;
    struct groupinfo *p;
    struct userinfo *q;
    q=userhead;
    p=grouphead;
    while(p)
    {
        if(strcmp(p->groupname,recv->data.message)==0)
        {
            ...
            if(strcmp(p->groupowner,recv->data.from)==0)
            {
                ...
                for(j=0;j<p->membernum;j++)
                {
                    if(strcmp(p->member[j],recv->data.to)==0)
                    {
                        recv->type='g';
                        biaot=1;
                        while(q)
                        {
                            if(strcmp(q->username,recv->data.to)==0)
                            {
                                locate=locateuser(recv->data.message,recv->data.to);
                                for(te=locate;te<q->numgroup;te++)
                                {
                                    strcpy(q->group[te],q->group[te+1]);
                                }
                                q->numgroup--;
                                break;
                            }
                        ...
                        locate=locategroup(recv->data.message,recv->data.to);
                        for(te=locate;te<p->membernum;te++)
                        {
                            strcpy(p->member[te],p->member[te+1]);
                        }
                        p->membernum--;
                        printf("\n群主%s已将%s移除群%s\n",recv->data.from,recv->data.message,recv->data.to);
                        break;
                    }
                }
            }
    ...
    if(groupn==0)
    {
        memcpy(recv->data.name[0],"g\0",2);
    }
    if(biaof==0)
    {
        memcpy(recv->data.name[0],"o\0",2);
    }
    if(biaot==0)
    {
        memcpy(recv->data.name[0],"p\0",2);
        ...
        case 'v':
            {
                ...
                while(p)
                {
                    if((strcmp(recv_pack->data.from,p->from)==0)&&(strcmp(recv_pack->data.to,p->to)==0)&&(strcmp(recv_pack->data.name[0],p->file)==0))
                    {
                        flag=1;
                        if(p->sign==0)
                        {
