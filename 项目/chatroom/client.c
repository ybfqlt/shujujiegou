...
void send_file()
{
    ...
    do
    {
        sendpack.data.nowsize=size;//保存文件当前大小
...
{
    int fd;
    if((fd=open(file.data.name[0],O_RDWR|O_CREAT|O_TRUNC|O_APPEND,S_IRWXU))==-1)
    {
        my_err("open",__LINE__);
        exit(1);
    }
    if(write(fd,file.data.message,strlen(file.data.message))!=strlen(file.data.message))
    {
